...
import com.yiranpay.quartz.biz.ReconciliationCheckBiz;
import com.yiranpay.quartz.biz.ReconciliationFileDownBiz;
import com.yiranpay.quartz.biz.ReconciliationFileParserBiz;
import com.yiranpay.quartz.biz.ReconciliationValidateBiz;
...
import com.yiranpay.reconciliation.enums.PublicStatusEnum;
import com.yiranpay.reconciliation.enums.ReleaseStatusEnum;
...
public class ReconciliationTask {
	...
	@Autowired
	private IRecBuildNoService recBuildNoService;
	...
	public void taskRun() {
		...
		try {
			...
			for (int num = 0; num < reconciliationInterList.size(); num++) {
				...
				// 创建对账批次
				batch.setCreater("yiranpaySystem");
				batch.setCreateTime(new Date());
				batch.setBillDate(billDate);
				String batchNo = recBuildNoService.buildReconciliationNo();
			...
			LOG.error("yiranpay-reconciliation error:", e);
		}
