package com.yiranpay.quartz.biz;
...
import com.yiranpay.reconciliation.enums.ReconciliationMistakeTypeEnum;
import com.yiranpay.reconciliation.enums.TradeStatusEnum;
			...
			for (ReconciliationEntityVo bankRecord : bankList) {
				...
				if (record.getInstOrderNo().equalsIgnoreCase(bankRecord.getBankOrderNo())) {
					...
					if (record.getAmount().compareTo(bankRecord.getBankAmount()) == 1) {
						...
						mistakeCount++;
						//更新入账流水
						String bizNo = record.getInstOrderNo();
						String compareFlag = "U"; 
						String compareBatchNo = batch.getBatchNo();
						String isWriteoff = "N";
						String billDate = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
						reconciliationAccountflowService.updateflow(bizNo,compareFlag,compareBatchNo,billDate,isWriteoff);
						break;
					}
					// 平台金额少
