...
import com.yiranpay.member.enums.OrderModeEnum;
import com.yiranpay.member.enums.TxnTypeEnum;
import com.yiranpay.member.request.AccBalanceListRequest;
import com.yiranpay.member.request.GetBalanceListReq;
import com.yiranpay.member.request.GetBalanceListRequestParam;
...
public class AccountDomainUtil {
	...
	public static AccountDomain convertToAccount(MemberTrMemberAccount acctDO,
        ...
        acct.setCateEnum(AccountCategoryEnum.getByCode(acctDO.getCategory()));
        acct.setAccountTypeId(acctDO.getTypeId());
        //new BigDecimal(mad.getWithdrawBalance()).setScale(2, BigDecimal.ROUND_DOWN)
        acct.setBalance(new BigDecimal(mad.getBalance()).setScale(2, BigDecimal.ROUND_DOWN));
        acct.setAvailableBalance(new BigDecimal(mad.getAvailableBalance()).setScale(2, BigDecimal.ROUND_DOWN));
        acct.setFrozenBalance(new BigDecimal(mad.getFrozenBalance()).setScale(2, BigDecimal.ROUND_DOWN));
        acct.setWithdrawBalance(new BigDecimal(mad.getWithdrawBalance()).setScale(2, BigDecimal.ROUND_DOWN));
