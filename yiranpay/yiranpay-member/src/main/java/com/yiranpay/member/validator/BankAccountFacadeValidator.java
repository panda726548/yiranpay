...
import com.yiranpay.member.enums.BankAccountAttrEnum;
import com.yiranpay.member.enums.BankAccountSignEnum;
import com.yiranpay.member.enums.BankAccountStatusEnum;
import com.yiranpay.member.enums.BankAccountTypeEnum;
import com.yiranpay.member.enums.BankAccountVerifiedEnum;
import com.yiranpay.member.enums.CertTypeEnum;
import com.yiranpay.member.enums.PayAttributeEnum;
...
import com.yiranpay.member.utils.ValidateUtils;
...
public class BankAccountFacadeValidator {
   ...
   /* public static void validatorQPayAccount(BankQPayAccountRequest request) {
    ...
    public static void validatorAddBankAccount(BankAccInfoRequest request) {
        ...
        if (StringUtils.isEmpty(request.getBankInfo().getIsSigning())) {
    ...
    private static void validateBankAccount(MemberTrBankAccount request, boolean isMust) {
        ...
        if (isMust && StringUtils.isEmpty(request.getBankId())) {
            throw new MaIllegalArgumentException("银行编号未提供");
        }
        if(isMust && StringUtils.isEmpty(request.getBankAccountNo())){
        ...
        if (isMust && StringUtils.isEmpty(request.getBankName())) {
        ...
        if (StringUtils.isNotEmpty(request.getIsFillCertNo())) {
            if (null == YesNoEnum.getByCode(request.getIsFillCertNo())) {
                throw new MaIllegalArgumentException("是否补全证件号码值非法");
            }
        }
        if (StringUtils.equals(YesNoEnum.YES.getCode(), request.getIsFillCertNo())
                && StringUtils.isBlank(request.getCertType())) {
        ...
        YesNoEnum yesEnum = StringUtils.isNotBlank(request.getIsFillCertNo()) ? YesNoEnum.getByCode(request.getIsFillCertNo())
                : YesNoEnum.NO;
        if (StringUtils.isNotBlank(request.getCertType()) && YesNoEnum.NO == yesEnum
   ...
   public static void checkBankAccInfoRequest(BankAccInfoRequest request) {
    ...
    public static void checkIsSigning(String isSingning) {
        ...
        if (StringUtils.isNotEmpty(isSingning) && null == BankAccountSignEnum.getByCode(isSingning)) {
...
//        }
    ...
    public static void checkSignNum(String signNum){
        if (StringUtils.isBlank(signNum)) {
