...
import com.yiranpay.member.enums.VerifyTypeEnum;
import com.yiranpay.member.exception.MaIllegalArgumentException;
import com.yiranpay.member.request.VerifyBindQueryRequest;
import com.yiranpay.member.request.VerifyInputRequest;
import com.yiranpay.member.request.VerifyQueryRequest;
...
public class VerifyFacadeValidator {
    ...
    private static void validatorVerifyEntityAndType(String verifyEntity, Integer verifyType,
                                                     boolean isMust) {
        if (isMust && StringUtils.isEmpty(verifyEntity)) {
            throw new MaIllegalArgumentException("认证实体未提供");
        } else {
            //如果修改时提供了认证类型必须提供认证实体
            if (verifyType != null && StringUtils.isEmpty(verifyEntity)) {
                throw new MaIllegalArgumentException("认证实体未提供");
            }
        }
        ...
        if (isMust && verifyType == null) {
            ...
            //如果修改时提供了认证实体必须提供类型
            if (StringUtils.isNotEmpty(verifyEntity) && verifyType == null) {
                throw new MaIllegalArgumentException("认证类型未提供");
            }
        }
    ...
    private static void validatorVerifyEntityAndTypeFormat(String verifyEntity, Integer verifyType){
      //验证实体的格式
        if (verifyType != null && StringUtils.isNotEmpty(verifyEntity)) {
...
//                }
    ...
    private static void validatorImgPath(String imgPath, boolean isMust) {
        if (isMust && StringUtils.isEmpty(imgPath)) {
