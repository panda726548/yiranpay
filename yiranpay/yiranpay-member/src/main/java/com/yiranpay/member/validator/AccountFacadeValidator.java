...
import com.yiranpay.member.enums.FrozenStatusEnum;
import com.yiranpay.member.enums.LifeCycleStatusEnum;
...
public class AccountFacadeValidator {
   ...
   /* *//**
    ...
    public static void validator(QueryBasicAccountRequest request){
        if(StringUtils.isBlank(request.getMemberId()) && StringUtils.isBlank(request.getMemberIdentity())){
    ...
    public static void validator(AccBalanceListRequest request) {
        if (StringUtils.isBlank(request.getAccountId()) && StringUtils.isBlank(request.getMemberId())) {
            throw new MaIllegalArgumentException("账户Id与会员Id不能都为空");
        }
        if (StringUtils.isBlank(request.getAccountId())
            && StringUtils.isNotBlank(request.getMemberId())) {
