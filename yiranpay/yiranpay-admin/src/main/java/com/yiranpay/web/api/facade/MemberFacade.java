...
public class MemberFacade extends BaseController{
   ...
   /*public ResultWrapper<Map<String,Object>> activateCompanyMember(ActivateCompanyRequest request) {
        try {
            ActivateCompanyResponse response = memberTmMemberService.activateCompanyMember(request);
            if (logger.isInfoEnabled()) {
     ...
     * @param request
     */
	@PostMapping("/createIntegratedCompanyMember")
    @ApiOperation(value = "集成创建企业会员",notes="集成创建企业会员")
    public ResultWrapper<Map<String,Object>> createIntegratedCompanyMember(@RequestBody IntegratedCompanyRequest request){
        try {
