...
(function ($) {
    $.extend({
    	...
    	modal: {
            ...
            // 弹出层全屏
            openNoBtnFull: function (title, url, width, height) {
            	//如果是移动端，就使用自适应大小弹窗
            	if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
            	    width = 'auto';
            	    height = 'auto';
        ...
        operate: {
         ...
         // 修改访问地址
          ...
          //设置目标机构setTargetInst
            setTargetInst: function(id) {
            	table.set();
            	if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
            		var row = $('#' + table.options.id).bootstrapTreeTable('getSelections')[0];
                	if ($.common.isEmpty(row)) {
            			$.modal.alertWarning("请选择资金渠道");
            			return;
            		}
                    var url = table.options.setTargetInstUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.openFull("资金渠道基本配置 - 维护资金源 - 设置目标机构", url);
            	} else {
            	    $.modal.openFull("资金渠道基本配置 - 维护资金源 - 设置目标机构" , $.operate.setTargetUrl(id));
            	}
            },
            setTargetUrl: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = table.options.setTargetInstUrl.replace("{id}", id);
            	} else {
            	    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
            	    if (id.length == 0) {
            			$.modal.alertWarning("请至少选择一条记录");
            ...
            setFundChannelExt: function(id) {
            	...
            	if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.setTargetInstUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.openTab("资金渠道基本配置 - 维护资金源 - 设置渠道特性", url);
            	} else {
            	    $.modal.openTab("资金渠道基本配置 - 维护资金源 - 设置渠道特性" , $.operate.setFundChannelExtUrl(id));
            	}
            },
            setFundChannelExtUrl: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = table.options.setFundChannelExtUrl.replace("{id}", id);
            	} else {
            	    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
            	    if (id.length == 0) {
            			$.modal.alertWarning("请至少选择一条记录");
          ...
          //设置渠道维护期
            setChannelMaintenancePeriod: function(id) {
            	table.set();
            	if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
            		var row = $('#' + table.options.id).bootstrapTreeTable('getSelections')[0];
                	if ($.common.isEmpty(row)) {
                    ...
                    var url = table.options.setTargetInstUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.openTab("资金渠道基本配置 - 维护资金源 - 设置渠道维护期", url);
            	} else {
            	    $.modal.openTab("资金渠道基本配置 - 维护资金源 - 设置渠道维护期" , $.operate.getChannelMaintenancePeriodUrl(id));
            	}
            },
            getChannelMaintenancePeriodUrl: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = table.options.setFundChannelExtUrl.replace("{id}", id);
            	} else {
            	    ...
            	    url = table.options.setChannelMaintenancePeriodUrl.replace("{id}", id);
            	}
                return url;
            },
            
          //设置渠道维接口
            setChannelApi: function(id) {
            	table.set();
            	if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
            		var row = $('#' + table.options.id).bootstrapTreeTable('getSelections')[0];
                	if ($.common.isEmpty(row)) {
                    ...
                    var url = table.options.setTargetInstUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.openTab("资金渠道基本配置 - 维护资金源 - 设置渠道接口", url);
            	} else {
            	    $.modal.openTab("资金渠道基本配置 - 维护资金源 - 设置渠道接口" , $.operate.getChannelApiUrl(id));
            	}
            },
            getChannelApiUrl: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = table.options.setFundChannelExtUrl.replace("{id}", id);
            	} else {
            	    ...
            	    url = table.options.setChannelApiUrl.replace("{id}", id);
            	}
                return url;
            },
            // 修改信息，以tab页展现
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.yiranpay_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
