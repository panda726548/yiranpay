...
import com.yiranpay.reconciliation.enums.HandleStatusEnum;
import com.yiranpay.reconciliation.enums.MistakeHandleStatusEnum;
...
import com.yiranpay.reconciliation.service.IReconciliationAccountCheckTransactionService;
import com.yiranpay.reconciliation.service.IReconciliationTradeReconciliationService;
...
public class ReconciliationAccountCheckTransactionServiceImpl implements IReconciliationAccountCheckTransactionService {

	private static final Log LOG = LogFactory.getLog(ReconciliationAccountCheckTransactionServiceImpl.class);

	@Autowired
	private IReconciliationAccountCheckBatchService rpAccountCheckBatchService;
	@Autowired
	private IReconciliationAccountCheckMistakeService rpAccountCheckMistakeService;
	@Autowired
	private IReconciliationAccountCheckMistakeScratchPoolService rpAccountCheckMistakeScratchPoolService;
	@Autowired
	private IReconciliationTradeReconciliationService rpTradeReconciliationService;

	/**
	 * 为了保证事务一致性，把对数据库的操作放入一个事务中
	 */
	@Transactional(rollbackFor = Exception.class)
	public void saveDatasaveDate(ReconciliationAccountCheckBatch batch, List<ReconciliationAccountCheckMistake> mistakeList, List<ReconciliationAccountCheckMistakeScratchPool> insertScreatchRecordList, List<ReconciliationAccountCheckMistakeScratchPool> removeScreatchRecordList) {
		...
		if (batch.getHandleStatus() == null) {
			batch.setHandleStatus(HandleStatusEnum.HANDLED.name());
		}
		rpAccountCheckBatchService.insertReconciliationAccountCheckBatch(batch);
	...
	public void handle(String id, String handleType, String handleRemark) {
		// 根据id查询
		ReconciliationAccountCheckMistake mistake = rpAccountCheckMistakeService.selectReconciliationAccountCheckMistakeById(Integer.parseInt(id));
		mistake.setHandleStatus(MistakeHandleStatusEnum.HANDLED.name());
		mistake.setHandleRemark(handleRemark);
		// 修改差错记录
		rpAccountCheckMistakeService.updateReconciliationAccountCheckMistake(mistake);
