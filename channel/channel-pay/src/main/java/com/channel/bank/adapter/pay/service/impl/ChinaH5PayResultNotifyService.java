 ...
 */
@Service
public class ChinaH5PayResultNotifyService {
	 private Logger logger = LoggerFactory.getLogger(ChinaH5PayResultNotifyService.class);
	 @Autowired
	 ...
	 public ChannelFundResult notify(ChannelRequest channelRequest) {
	        ...
	        try {
	            ...
	            //支付异步通知
				if("TRADE_SUCCESS".equals(fundResponse.get("status"))) {
             		String apiResultCode = "{\"return_code\":\"SUCCESS\",\"return_msg\":\"OK\"}";
             		channelResult = buildVerifySignResult(
             				respData, 
             				fundResponse.get("merOrderId"), 
             				AmountUtils.Fen2Yuan(Long.parseLong(fundResponse.get("totalAmount"))),
             				"0000", 
             				fundResponse.get("targetOrderId"), 
             				null,
