 ...
 */
@RestController
@RequestMapping("/api/yiranpay/channelpay/aliapppay")
public class ALIAppPayFundinFacade extends BasePayFundinFacade {

	private Logger logger = LoggerFactory.getLogger(ALIAppPayFundinFacade.class);
	...
	public ResultWrapper<Map<String,Object>>  fundin(@RequestBody String request) {
        ...
        try {
            ...
            logger.info("支付宝支付响应参数：{}", JSONUtil.toJsonStr(alipayTradeAppPayResponse));
            if (alipayTradeAppPayResponse.isSuccess()) {
            	logger.info("body:"+alipayTradeAppPayResponse.getBody());
    			...
    			result.setFromHtml(alipayTradeAppPayResponse.getBody());
    			result.setExtension(JSON.toJSONString(alipayTradeAppPayResponse));
                return ResultWrapper.ok().putData(result);
            } else {
